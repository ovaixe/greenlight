...
// Declare a custom Runtime type, which has the underlying type inte2 (the same as our Movie struct field).
type Runtime int32
...
func (r *Runtime) UnmarshalJSON(jsonValue []byte) error {
	...
	// Convert the int32 to a Runtime type and assing this to the receiver. Note that we use the * operator to deference the receiver (which is a pointer to a Runtime type) in order to set the underlying value of the pointer.
	*r = Runtime(i)
